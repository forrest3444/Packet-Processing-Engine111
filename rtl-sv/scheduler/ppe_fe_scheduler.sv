...
module ppe_fe_scheduler (
    ...
    always_comb begin : d2a0_shortlist
        ...
        for (int bank = 0; bank < FE_NUM; bank++) begin
            ...
            way0_slot = cand_id_t'(bank);
            way1_slot = cand_id_t'(FE_NUM + bank);

            if (!candidate_way_rr_q[bank]) begin
                if (candidate_available[way0_slot]) begin
                    shortlist_valid[bank]        = 1'b1;
                    shortlist_candidate_id[bank] = way0_slot;
                    shortlist_seq_tag[bank] =
                        candidate_seq_tag_i[bank];
                    shortlist_delay[bank] =
                        candidate_delay_i[bank];
                end else if (candidate_available[way1_slot]) begin
                    shortlist_valid[bank]        = 1'b1;
                    shortlist_candidate_id[bank] = way1_slot;
                    shortlist_seq_tag[bank] =
                        candidate_seq_tag_i[FE_NUM + bank];
                    shortlist_delay[bank] =
                        candidate_delay_i[FE_NUM + bank];
                end
            end else begin
                if (candidate_available[way1_slot]) begin
                    shortlist_valid[bank]        = 1'b1;
                    shortlist_candidate_id[bank] = way1_slot;
                    shortlist_seq_tag[bank] =
                        candidate_seq_tag_i[FE_NUM + bank];
                    shortlist_delay[bank] =
                        candidate_delay_i[FE_NUM + bank];
                end else if (candidate_available[way0_slot]) begin
                    shortlist_valid[bank]        = 1'b1;
                    shortlist_candidate_id[bank] = way0_slot;
                    shortlist_seq_tag[bank] =
                        candidate_seq_tag_i[bank];
