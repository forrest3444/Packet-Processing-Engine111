...
module PPE_INGRESS #(
    ...
    output wire                              bkps_o,
    // The registered FIFO head is presented as a reservation mask.  The ROB
    // returns a combinational ready decision for this exact batch.
    output wire [`PPE_N-1:0]                 alloc_reserve_valid_o,
    ...
    always @* begin : slot_head_select
        integer lane_idx;
        reg [LANE_COUNT_W-1:0] lower_pair_count;
        reg [LANE_COUNT_W-1:0] upper_pair_count;

        head_lane_valid       = {N{1'b0}};
        head_packet_count     = {LANE_COUNT_W{1'b0}};
        head_dep_required     = {N{1'b0}};
        lower_pair_count      = {LANE_COUNT_W{1'b0}};
        upper_pair_count      = {LANE_COUNT_W{1'b0}};
        ...
        if (slot_count_q != {SLOT_COUNT_W{1'b0}}) begin
            ...
            if (slot_rd_ptr_q) begin
                lower_pair_count =
                    {{(LANE_COUNT_W-1){1'b0}}, slot_lane_valid_q[1][0]}
                    + {{(LANE_COUNT_W-1){1'b0}}, slot_lane_valid_q[1][1]};
                upper_pair_count =
                    {{(LANE_COUNT_W-1){1'b0}}, slot_lane_valid_q[1][2]}
                    + {{(LANE_COUNT_W-1){1'b0}}, slot_lane_valid_q[1][3]};
            end else begin
                lower_pair_count =
                    {{(LANE_COUNT_W-1){1'b0}}, slot_lane_valid_q[0][0]}
                    + {{(LANE_COUNT_W-1){1'b0}}, slot_lane_valid_q[0][1]};
                upper_pair_count =
                    {{(LANE_COUNT_W-1){1'b0}}, slot_lane_valid_q[0][2]}
                    + {{(LANE_COUNT_W-1){1'b0}}, slot_lane_valid_q[0][3]};
            end
    ...
    assign alloc_reserve_valid_o = head_lane_valid;
