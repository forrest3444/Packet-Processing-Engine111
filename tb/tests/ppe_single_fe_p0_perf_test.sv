//------------------------------------------------------------------------------
// P0 characterization for the single-FE baseline.
//
// Uses the production P0 sequence and scoreboard. Backpressure is measured,
// not rejected, because it is intrinsic to the intentionally narrow baseline.
//------------------------------------------------------------------------------

import uvm_pkg::*;
import ppe_tb_pkg::*;

class ppe_single_fe_p0_perf_test extends uvm_test;
    `uvm_component_utils(ppe_single_fe_p0_perf_test)
    ...
    function void build_phase(uvm_phase phase);
        ...
        if (!uvm_config_db #(ppe_vif_t)::get(this, "", "vif", vif)) begin
            `uvm_fatal("NOVIF", "single-FE P0 test cannot get vif")
        end
    endfunction
    ...
    task collect_metrics();
        ...
        repeat (TIMEOUT_CYCLES) begin
            @(posedge vif.clk);
            cycle_count++;
            accepted_now = !vif.bkps
                ? count4({vif.in_valid3, vif.in_valid2,
            ...
            if (measuring) begin
                ...
                repeat (retired_now) begin
                    if (accept_cycle_q.size() == 0) begin
                        `uvm_error("P0_LATENCY",
                                   "output has no acceptance timestamp")
                    end else begin
    ...
    task check_results();
        ...
        if (accept_cycle_q.size() != 0) begin
            `uvm_error("P0_PENDING", $sformatf(
                "%0d acceptance timestamps remain", accept_cycle_q.size()))
        end
    ...
    task report_metrics();
        ...
        `uvm_info("P0_PERF", $sformatf(
            "rob_occupancy_avg=%.2f rob_occupancy_peak=%0d",
